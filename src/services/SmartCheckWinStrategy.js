const strategy = ({board, r, c, player}) => {
   let discs = 0;
    ...
    discs = 1;
    for ( let i = 1; r-i >= 0 && c - i >= 0 && board[r- i][c - i] == player; ++i) {
        discs++;
    }
    // ... and from current chip to top right
    for ( let i = 1; r + i < board.length && c + i < board[r + i].length
            && board[r+ i][c + i] == player; ++i) {
        discs++;
    }
    if (discs >= 4) {
        console.log('player won', player);
    ...
    discs = 1;
    for ( let i = 1; r - i >= 0 && c + i < board[r - i].length && board[r- i][c + i] == player; ++i) {
        discs++;
    }
    // ... and from current chip to top left
    for ( let i = 1; r + i < board.length && c - i >= 0 && board[r + i][c - i] == player; ++i) {
        discs++;
    }
    if (discs >= 4) {
        console.log('player won', player);
